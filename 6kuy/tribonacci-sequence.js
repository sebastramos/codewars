...
// [0, 0, 1, 1, 2, 4, 7, 13, 24, ...]
// Well, you may have guessed it by now, but to be clear: you need to create a fibonacci function that given a signature array, returns the first n elements - signature included of the so seeded sequence.
...
function tribonacci(signature,n){
	if (n === 0) {
  	...
  	for(let i = 0; i < n - 3; i++) {
  		signature.push(signature.slice(i, signature.length).reduce((a,b)=> a + b, 0))
  	}
  	return signature
	}
}
